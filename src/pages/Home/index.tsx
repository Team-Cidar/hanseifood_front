    ...
    <>
      {isModalOpen && (
        <Modal
          ...
          body={Object.entries(weeklyData.additionalMenu.menus).map((res, key) => {
            return (
              <div key={key}>
                <div>{res[0]}</div>
                <div>
          ...
          bottom={<IconButton width={'84'} height={'32'} onClick={handleModal} label={'닫기'} />}
        />
      )}
      {/* {isFeedbackModal && <Modal header={'댓글'} body={<div>asdfas</div>} bottom={<div>asdfs</div>} />} */}
      <HomeView weeklyData={weeklyData} toggleHandler={toggleHandler} handleModal={handleModal} loading={loading} />
    </>
  );
};
