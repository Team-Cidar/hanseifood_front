...
const MenuSpecificItem = ({ menu, liked, onInteraction }: MenuSpecificItemProps) => {
  return (
    <Container onClick={() => onInteraction(menu.menuId)}>
      {menu.deleted ? <DateText $deleted>{menu.date}</DateText> : <DateText>{menu.date}</DateText>}
      <MenuCard>
      ...
      <FeedbackBottom>
        <SvgView>
          ...
          <SvgText>{menu.commentCount}</SvgText>
        </SvgView>
        <SvgView>
          <SvgIcon name={'like'} width={14} height={14} fill={liked ? EColor.RED : EColor.TEXT_500} />
          <SvgText>{menu.likeCount}</SvgText>
        </SvgView>
      </FeedbackBottom>
    </Container>
  );
